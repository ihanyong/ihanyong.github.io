...
| Time-windowed Join                  | B,S                 | Note: Time-Windowed Joins 可以流的方式处理常规连接的一个子集<br/>时间窗口连接要求至少一个等值连接的谓词，和一个限定连接两端的时间的连接条件。这个条件可以通过两个范围条件谓词(<,<=,>=,>)，或是一个等值条件谓词来比较两张表的相同类型的时间属性(基于处理时间或事件时间)<br/>如下面两个条件是有效的窗口连接条件：<br/><p>ltime === rtime <br/>ltime >= rtime && ltime < rtime + 10.minutes</p><br/><p>Table left = tableEnv.fromDataSet(ds1, "a, b, c, ltime.rowtime"); <br/>Table right = tableEnv.fromDataSet(ds2, "d, e, f, rtime.rowtime"); Table result = left.join(right) .where("a = d && ltime >= rtime - 5.minutes && ltime < rtime + 10.minutes") .select("a, b, e, ltime");</p> |
| Inner Join with Table Function      | B,S                 | 根据表函数的结果来连接表。左表的每条记录被连接到调用的表函数产生的全部记录。 对于左表的一条记录，如果用他调用表函数返回的结果是空集，则该记录被丢弃。 <br/><p> ``` // register User-Defined Table Function TableFunction<String> split = new MySplitUDTF(); tableEnv.registerFunction("split", split); // join Table orders = tableEnv.scan("Orders"); Table result = orders .join(new Table(tableEnv, "split(c)").as("s", "t", "v")) .select("a, b, s, t, v"); ​```</p> |
| Left Outer Join with Table Function | B,S                 | 根据表函数的结果来连接表。左表的每条记录被连接到调用的表函数产生的全部记录。 对于左表的一条记录，如果用他调用表函数返回的结果是空集，则该记录会被保留，并填充null值。 <br/><p> Note: 当前，表函数左外部联接的谓词结果只能为空或字符串"true"。 ``` // register User-Defined Table Function TableFunction<String> split = new MySplitUDTF(); tableEnv.registerFunction("split", split); // join Table orders = tableEnv.scan("Orders"); Table result = orders .leftOuterJoin(new Table(tableEnv, "split(c)").as("s", "t", "v")) .select("a, b, s, t, v"); ``` </p>|
| Join with Temporal Table            | B,S                 | 历史表是随着时间追踪的变更的表。 <br/><br/>历史表函数允许访问在指定时间点上历史表的状态。 连接历史表的语法与内连接一个表函数是一样的。 当前历史表只支持内连接。 ``` Table ratesHistory = tableEnv.scan("RatesHistory"); // register temporal table function with a time attribute and primary key TemporalTableFunction rates = ratesHistory.createTemporalTableFunction("r_proctime", "r_currency"); tableEnv.registerFunction("rates", rates); // join with "Orders" based on the time attribute and key Table orders = tableEnv.scan("Orders"); Table result = orders .join(new Table(tEnv, "rates(o_proctime)"), "o_currency = r_currency") ```</p> |
