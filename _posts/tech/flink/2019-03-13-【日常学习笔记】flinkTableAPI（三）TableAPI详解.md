...
| Where/Filter | B,S  | 类似于SQL的Where。 过滤掉不符合条件的行。<br/>```Table orders = tableEnv.scan("Orders"); Table result = orders.where("b === 'red'");```<br/>```orTable orders = tableEnv.scan("Orders"); Table result = orders.filter("a % 2 === 0");``` |
